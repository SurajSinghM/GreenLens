...
// Simple Vercel Serverless Function to proxy requests to the Lovable/Gemini AI gateway.
// It reads the server-side secret LOVABLE_API_KEY from process.env (do NOT prefix with VITE_)
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!LOVABLE_API_KEY) {
      return res.status(500).json({ error: 'LOVABLE_API_KEY not configured on the server' });
    }
    ...
    const response = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: 'user', content: `Analyze this product: ${JSON.stringify(productData)}` },
        ],
        temperature: 0.7,
      }),
    });
    ...
    const aiResponse = data.choices?.[0]?.message?.content ?? data;
